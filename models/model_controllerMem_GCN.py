...
class model_controllerMem_gcn(nn.Module):
    ...
    def __init__(self, settings, model_pretrained):
        ...
        self.conv2 = model_pretrained.conv2
    ...
    def init_memory(self, data_train):
        ...
        for i in range(self.num_prediction + 1):
            ...
            # kitti 9, INTERACTION 8
            x = data_train[j][8]['x']
            edge_index = data_train[j][8]['edge_index']
            edge_weight = data_train[j][8]['edge_weight']
            # past = past.cuda()
            # future = future.cuda()
            ...
            story_embed = torch.transpose(story_embed, 1, 2)
            output_past, surr_past = self.encoder_past(story_embed)

            # GCN

            if edge_weight.size()[0] != 0:
                x1 = surr_past.view(surr_past.size()[1], surr_past.size()[2])
                ...
                interact_past = x1[0].view(1, 1, -1)
            else:
                interact_past = torch.zeros(1, 1, 48)

            # incorporate target past encoding with interaction encoding
            # co_past = interact_past + state_past
            co_past = torch.cat((interact_past, state_past), 2)

            # insert in memory
            self.memory_past = torch.cat((self.memory_past, co_past.squeeze(0)), 0)
    ...
    def check_memory(self, index):
        ...
        zero_padding = torch.zeros(1, 1, self.dim_embedding_key * 3)  # .cuda()
    ...
    def forward(self, past, x, edge_index, edge_weight, future=None):
        """
        Forward pass.
        ...
        state_normalized = F.normalize(co_past.squeeze(dim=1), p=2, dim=1)
        ...
        for i_track in range(self.num_prediction):
            ...
            info_total = torch.cat((co_past, info_future.unsqueeze(0)), 2)
        ...
        if future is not None:
            ...
            past_to_write = co_past.squeeze(dim=1)[index_writing]
            future_to_write = state_fut.squeeze(dim=1)[index_writing]
    ...
    def write_in_memory(self, past, future, x, edge_index, edge_weight):
        """
        ...
        # Cosine similarity and memory read
        past_normalized = F.normalize(self.memory_past, p=2, dim=1)
        state_normalized = F.normalize(co_past.squeeze(dim=0), p=2, dim=1)
        ...
        index_writing = np.where(writing_prob.cpu() > 0.5)[0]
        past_to_write = co_past.squeeze(dim=0)[index_writing]
