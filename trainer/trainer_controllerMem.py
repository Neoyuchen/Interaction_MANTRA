...
class Trainer():
    ...
    def evaluate(self, loader, epoch=0):
        ...
        with torch.no_grad():
            ...
            iters = 0
            for step, (index, past, future, presents, angle_presents, videos, vehicles, number_vec, scene) in enumerate(loader):
                iters = iters + 1
                past = Variable(past)
    ...
    def _train_single_epoch(self):
        ...
        """
        config = self.config
        for step, (index, past, future, _, _, _, _, _, _) in enumerate(tqdm.tqdm(self.train_loader)):
            self.iterations += 1
            past = Variable(past)
            future = Variable(future)
            if config.cuda:
