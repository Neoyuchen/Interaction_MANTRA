...
class Trainer:
    def __init__(self, config):
        ...
        self.folder_tensorboard = 'runs/runs-ae-itr/'
        self.folder_test = 'training/training_ae_itr/' + self.name_test + '_' + config.info
        ...
        tracks_train = pd.read_csv(config.dataset_file)
        tracks_test = pd.read_csv(r"D:\data\INTERACTION-Dataset-DR-multi-v1_2\val\DR_USA_Roundabout_FT_val.csv")
        self.dim_clip = 180
        self.data_train = dataset_invariance.InteractionDataset(tracks_train,
        ...
        self.data_test = dataset_invariance.InteractionDataset(tracks_test,
                                                         len_past=config.past_len,
                                                         len_future=config.future_len,
                                                         train=False,
                                                         graph_train=True,
        ...
        if os.path.isfile(config.pretrained_model):
            self.mem_n2n = torch.load(config.pretrained_model)
    ...
    def evaluate(self, loader, epoch=0):
        ...
        # Loop over samples
        # , scene, scene_one_hot
        for step, (index, past, future, presents, angle_presents, videos, vehicles, number_vec) \
